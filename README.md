...
AI 写代码做视频，**美感天花板很低**，做不了品牌片；但**信息型 / 数据型视频**（KPI、参数、数据展示）是代码生成的甜区——对手是 Excel / PPT，不是 AE / 设计师。本工具专注这个甜区：用确定性渲染 + 中文配音，把"数据"变成可播放的短视频。支持 `cover` / `cards` / `bars` / `table` 多版式汇报片。

## 成片示例

`mvp/renders/final.mp4` —— 3 页 KPI 轮播带中文配音，约 5s（虚构穿戴设备示例数据）。

工具支持 `cover` / `cards` / `bars` / `table` / `kpis` 五种版式，可混合编排成完整汇报片（封面 + 卡片 + 进度条 + 表格）。修改 `scene.json` 的 `type` 字段即可切换，**无需改任何代码**。
...
- `render.mjs`：编排层。读 pages → 每页 TTS → 算时间 plan → 按 `type` 生成内容区 + N 个 section 注入模板 → 渲染 → 混音。
- `templates/index.template.html`：数据驱动模板。`window.__PLAN` 注入后，通用遍历 pages 构建 GSAP timeline（cover / cards / bars / table / kpis 通用入场）。
...
| type | 内容字段 | 用途 |
|------|----------|------|
| `cover` | `subtitle`（支持 `<b>` 高亮） | 封面 / 收尾大标题页 |
| `cards` | `cards[]`（`icon` / `title` / `desc` / `tag` / `highlight`） | 卡片网格，≥ 6 张自动转 3 列布局 |
| `bars` | `bars[]`（`label` / `pct` / `tone`） | 进度条，`tone`: `ok` / `warn` / `bad` |
| `table` | `head[]` / `rows[].cells[]`（cell 支持 `{text,tone}` 或 `{html,tone}`） | 表格，富单元格可嵌色点 / 标签 |
| `kpis` | `kpis[]`（`val` / `unit` / `label` / `desc`） | KPI 数据（向后兼容，缺省 type 默认此项） |
...
```json
{
  ...
  "accent": "#00E5A0",
  "cards": [
    { "icon": "❤️", "title": "心率监测", "desc": "24 小时实时心率与异常预警", "highlight": true, "tag": "核心" },
    { "icon": "🏃", "title": "运动追踪", "desc": "120 种运动模式与 GPS 轨迹" }
  ],
  "voice": "三大核心场景。"
}
```

> `scene.json` 为受控配置：需内联 HTML 的字段（cover `subtitle`、table 富单元格 `cell.html`）原样输出不转义，纯文本字段仍转义。
